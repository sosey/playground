...
class Imexam(GingaPlugin.LocalPlugin):
    ...
    def __init__(self, fv, fitsimage):
        ...
        self.imex = Imexamine()
        # remove the sys.stdout logger
        for handler in self.imex.log.handlers:
            if isinstance(handler, logging.StreamHandler):
                self.imex.log.removeHandler(handler)
    ...
    def imexam_cmd(self, canvas, keyname, data_x, data_y, func):
        ...
        try:
            ...
            self.msg_res.append_text(self.out_f.getvalue(), autoscroll=True)
        except Exception as e:
            # get any partial output
            self.msg_res.append_text(e.message)
